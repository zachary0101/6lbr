...
cetic_6lbr_init(void)
{
  ...
  uip_create_linklocal_prefix(&loc_fipaddr);	//fe80:0:0:0::
  uip_ds6_set_addr_iid(&loc_fipaddr, &uip_lladdr);	//EUI-64
  cetic_dag = rpl_set_root(RPL_DEFAULT_INSTANCE, &loc_fipaddr);

  uip_ds6_addr_t *local = uip_ds6_get_link_local(-1);//any link local address

  uip_ipaddr_copy(&wsn_ip_local_addr, &local->ipaddr);//put local addr into wsn_ip_local_addr(128bit)
...
/* ROUTER */
  ...
  if((nvm_data.mode & CETIC_MODE_ROUTER_SEND_CONFIG) != 0) {
    ...
    uip_ds6_prefix_add(&eth_net_prefix, 64, 1,
                       ...
                       30000, 30000);		//add to prefix list
  } else {
  ...
  uip_ds6_defrt_add(&eth_dft_router, 0);		//add to default router list
  ...
  uip_create_linklocal_prefix(&eth_ip_local_addr);	//fe80:0:0:0
